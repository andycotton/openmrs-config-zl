...
select 
dossier_id "# Dossier",
first_name "Code Prénatal",
last_name "Prénom(s) Nom(s) (Surnoms)",
mothers_first_name "Prénom de la mère",
DATE_FORMAT(date_of_birth,'%d/%m/%Y') "Date de Naissance",
age_at_encounter "Âge",
address "Lieu de résidence",
DATE_FORMAT(date_of_last_menstrual_period,'%d/%m/%Y') "Date Prénatal Dernières (DDR)",
gestational_age "Âge gestationnel (en semaine)",
DATE_FORMAT(estimated_delivery_date,'%d/%m/%Y') "Date Probable d'Accouchement (DPA)",
gravida "Gravida (G)",
parity "Para ou Parité (P)",
abortus "Aborta (A)",
living_children "Enfant(s) vivant(s) (EV)",
referral_source "Source de référence",
visit_count "# de visites",
DATE_FORMAT(visit_date,'%d/%m/%Y') "Date visite",
anti_tetanus_vaccination "Vaccination Antitétanique",
high_risks_for_the_pregnancy "Risques élevés les à grossesse",
DATE_FORMAT(pre_counseling_date,'%d/%m/%Y') "Date counseling pré test",
DATE_FORMAT(hiv_test_date,'%d/%m/%Y') "Date test (VIH)",
DATE_FORMAT(hiv_test_result_date,'%d/%m/%Y') 'Date résultat (VIH)',
result "Résultat (VIH)",
not_tested_for_hiv "Non testée pour (VIH)",
syphillis_details_not_tracked_in_emr "détails de la syphilis pas suivis dans l'EMR",
DATE_FORMAT(syphiliss_treatment_start_date,'%d/%m/%Y') "Date de début du traitement syphilis",
DATE_FORMAT(syphillis_treatment_end_date,'%d/%m/%Y') "Date de fin du traitement la syphilis",
on_art_for_hiv_prior_to_pregnancy "En soins ARV avant grossesse",
additional_hiv_details_not_in_the_emr "détails supplémentaires  VIH pas suivis dans le EMR",
weight_in_kg "Poids (en Kg)",
height_in_m "Taille (en m)",
upper_arm_circumference "Périmètre Brachial (PB)",
malnutrition_malaria_details_not_in_emr "détails malnutrition et paludisme pas suivis dans le EMR",
malnutrition "malnutrition",
iron_deficiency_anemia "Anémie ferriprive",
malaria "Malaria",
birth_plan "Plan d'accouchement élaboré",
accepts_accompanateur "Acceptation accompagnateur",
enrolled_in_mother_support_group "Inscription club de mères"
